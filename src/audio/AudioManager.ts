...
export class AudioManager {
  ...
  playSoundAt(name: string, position: Vector3, volume?: number): void {
    ...
    if (sound) {
      ...
      sound.play();
      console.log(`[AudioManager] Played sound "${name}" at position`, position);
    } else {
      console.warn(`[AudioManager] Sound "${name}" not found in cache. Available sounds:`, Array.from(this.sounds.keys()));
    }
  }
