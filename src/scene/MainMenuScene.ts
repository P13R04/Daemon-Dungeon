import { Scene, Engine, FreeCamera, Vector3, Color4, Sound } from '@babylonjs/core';
import {
...
export class MainMenuScene {
  ...
  constructor(
    ...
    // Initialize audio manager for UI sounds
  ...
  private async preloadUISounds(): Promise<void> {
    ...
    for (const sound of uiSounds) {
      try {
        await this.audioManager.loadSound(sound.id, sound.path, { autoplay: false });
        console.log(`[MainMenu] Loaded UI sound: ${sound.id}`);
      } catch (err) {
        console.warn(`[MainMenu] Failed to load ${sound.id}:`, err);
      }
    }
    
    // Bind EventBus sound events
    this.eventBus.on(GameEvents.UI_SOUND_SELECT, () => this.playUISound('sfx_ui_select1'));
    this.eventBus.on(GameEvents.UI_SOUND_DESELECT, () => this.playUISound('sfx_ui_deselect'));
    this.eventBus.on(GameEvents.UI_SOUND_START_GAME, () => this.playUISound('sfx_ui_start_game'));
    this.eventBus.on(GameEvents.UI_SOUND_NEXT_ROOM, () => this.playUISound('sfx_ui_next_room'));
  }

  private playUISound(soundId: string, volume: number = 0.8): void {
    console.log(`[MainMenu] Playing sound: ${soundId}`);
    try {
  ...
  private createMainButtons(): void {
    ...
    const playBtn = this.makeActionButton('menuPlay', 'START RUN', topOffsets[0], () => {
      this.eventBus.emit(GameEvents.UI_SOUND_START_GAME);
      this.hidePanels();
  ...
  private createSettingsOverlay(): void {
    ...
    this.bindButtonAction(closeBtn, () => {
      this.eventBus.emit(GameEvents.UI_SOUND_DESELECT);
      this.awaitingRebind = null;
      this.closeSettingsOverlay();
    });
