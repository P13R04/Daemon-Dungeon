...
export class GameManager {
  ...
  private async initializeGameplayScene(): Promise<void> {
    ...
    // Initialize audio manager and load monster sounds
    this.audioManager = new AudioManager(this.scene);
    // Bind listeners immediately (sounds may still be loading)
  ...
  private applyAudioSettings(settings: GameSettings): void {
    ...
    if (this.audioManager) {
      const effectiveVolume = GameSettingsStore.getEffectiveVolume('sfx');
      this.audioManager.setSFXVolume(effectiveVolume);
      this.audioManager.setMasterVolume(settings.audio.master);
    }
  }
  ...
  /**
   * Preload all monster sound effects
   * Follows the pattern: sfx/monsters/{monsterType}/{soundName}.mp3
   */
  private async preloadMonsterSounds(): Promise<void> {
    if (!this.audioManager) return;

    // Define all monster sounds to preload
    const monsterSounds = [
      // Bull sounds
      ...
      { id: 'sfx_jumper_dmg_taken2', path: 'sfx/monsters/jumper/jumper_dmg_taken2.mp3' },
    ...
    const loadPromises = monsterSounds.map(sound =>
      this.audioManager!.loadSound(sound.id, sound.path, { autoplay: false })
        .then(() => {
          console.log(`[Audio] ✓ Loaded ${sound.id} from ${sound.path}`);
          return sound.id;
        })
        .catch(err => {
          console.warn(`[Audio] ✗ Failed to load ${sound.id} from ${sound.path}:`, err);
          return null;
        })
    );
    ...
    try {
      ...
      console.log(`[GameManager] Monster sounds: ${loaded}/${monsterSounds.length} loaded successfully`);
      if (loaded < monsterSounds.length) {
        console.warn(`[GameManager] Some monster sounds failed to load`);
      }
    } catch (err) {
      console.error('Error preloading monster sounds:', err);
    }
  }
  ...
  private async preloadUISounds(): Promise<void> {
    ...
    const loadPromises = uiSounds.map(sound =>
      this.audioManager!.loadSound(sound.id, sound.path, { autoplay: false })
        .then(() => {
          console.log(`[Audio] ✓ Loaded ${sound.id} from ${sound.path}`);
          return sound.id;
    ...
    try {
      const results = await Promise.all(loadPromises);
      const loaded = results.filter(r => r !== null).length;
      console.log(`[GameManager] UI sounds: ${loaded}/${uiSounds.length} loaded successfully`);
    } catch (err) {
      console.error('Error preloading UI sounds:', err);
    }
  }

  /**
   * Bind UI sounds to game events
   */
  private bindUISoundEvents(): void {
    if (!this.audioManager) return;

    this.eventBus.on(GameEvents.UI_SOUND_SELECT, () => {
      this.audioManager?.playSound('sfx_ui_select1', 0.8);
    });

    this.eventBus.on(GameEvents.UI_SOUND_DESELECT, () => {
      this.audioManager?.playSound('sfx_ui_deselect', 0.8);
    });

    this.eventBus.on(GameEvents.UI_SOUND_START_GAME, () => {
      this.audioManager?.playSound('sfx_ui_start_game', 0.8);
    });

    this.eventBus.on(GameEvents.UI_SOUND_NEXT_ROOM, () => {
      this.audioManager?.playSound('sfx_ui_next_room', 0.8);
    });
  }

  /**
   * Bind monster sounds to game events
   */
  private bindMonsterSoundEvents(): void {
    if (!this.audioManager) return;

    // Monster damage taken sounds (when hit)
    this.eventBus.on(GameEvents.ENEMY_DAMAGED, (data: any) => {
      ...
      const sound = soundMap[enemyType] || (enemyType?.includes('pong') ? 'sfx_pong_onhit' : undefined);
      if (sound) {
        try {
          console.log(`[Audio] Playing ${sound} for ${enemyType} damage at`, data?.position);
          this.audioManager.playSoundAt(sound, data?.position, 0.7);
        } catch (err) {
          console.warn(`Failed to play monster damage sound ${sound}:`, err);
        }
      }
    });
    ...
    this.eventBus.on(GameEvents.ENEMY_BULL_AUDIO_CUE, (data: any) => {
      ...
      const cueSoundMap: Record<string, string> = {
        aim: 'sfx_bull_charge1',
        dash: 'sfx_bull_dash1',
        collision: 'sfx_bull_collision1',
      };
      const sound = cueSoundMap[cue];
      if (!sound) return;

      try {
        if (cue === 'aim') {
          this.audioManager.stopSound('sfx_bull_dash1');
          this.audioManager.stopSound('sfx_bull_collision1');
        }
        if (cue === 'dash') {
          this.audioManager.stopSound('sfx_bull_charge1');
          this.audioManager.stopSound('sfx_bull_collision1');
        }
        if (cue === 'collision') {
          this.audioManager.stopSound('sfx_bull_charge1');
        ...
        this.audioManager.playSoundAt(sound, position, 0.8);
      } catch (err) {
        console.warn(`Failed to play bull cue sound ${sound}:`, err);
      }
    });
    ...
    this.eventBus.on(GameEvents.ENEMY_JUMPER_AUDIO_CUE, (data: any) => {
      ...
      const cueSoundMap: Record<string, string> = {
        jump: 'sfx_jumper_jump1',
        land: 'sfx_jumper_land1',
      };
      const sound = cueSoundMap[cue];
      if (!sound) return;
      ...
      try {
        if (cue === 'jump') {
          this.audioManager.stopSound('sfx_jumper_land1');
        }
        if (cue === 'land') {
          this.audioManager.stopSound('sfx_jumper_jump1');
        }
        this.audioManager.playSoundAt(sound, position, 0.8);
      } catch (err) {
        console.warn(`Failed to play jumper cue sound ${sound}:`, err);
      }
    });
    ...
    this.eventBus.on(GameEvents.ENEMY_PONG_AUDIO_CUE, (data: any) => {
      ...
      const cueSoundMap: Record<string, string> = {
        bounce: 'sfx_pong_wall_bounce1',
        flyby: 'sfx_pong_flying_close1',
      };
      const sound = cueSoundMap[cue];
      if (!sound) return;
      ...
      try {
        if (cue === 'bounce') {
          this.audioManager.stopSound('sfx_pong_flying_close1');
        }
        this.audioManager.playSoundAt(sound, position, 0.8);
      } catch (err) {
        console.warn(`Failed to play pong cue sound ${sound}:`, err);
      }
    });

    this.eventBus.on(GameEvents.ENEMY_ARTIFICIER_SPLIT_IMPACT, (data: any) => {
      if (!this.audioManager) return;
      if (this.aliveArtificierCount <= 0) return;
      const position = data?.position || new Vector3(0, 0, 0);
      try {
        if (this.activeArtificierZoneIds.size === 0) {
          this.audioManager.stopSound('sfx_artificier_splash_zone_hum1');
          this.audioManager.playSoundAt('sfx_artificier_splash_zone_hum1', position, 0.55);
        }
        this.audioManager.playSoundAt('sfx_artificier_shot_split1', position, 0.85);
      } catch (err) {
        console.warn('Failed to play artificier split impact sound:', err);
      }
    });
    ...
    this.eventBus.on(GameEvents.ENEMY_ARTIFICIER_SHOT_FIRED, (data: any) => {
      ...
      const position = data?.position || new Vector3(0, 0, 0);
      try {
        this.audioManager.playSoundAt('sfx_sentry_shots_fired1', position, 0.85);
      } catch (err) {
        console.warn('Failed to play artificier shot fired sound:', err);
      }
    });
    ...
    this.eventBus.on(GameEvents.ENEMY_ARTIFICIER_PROJECTILE_FLIGHT_STARTED, (data: any) => {
      ...
      if (this.activeArtificierProjectilesInFlight === 1) {
        const position = data?.position || new Vector3(0, 0, 0);
        try {
          this.audioManager.playSoundAt('sfx_sentry_projectile_flying1', position, 0.5);
        } catch (err) {
          console.warn('Failed to play artificier projectile flight sound:', err);
        }
      }
    });
    ...
    this.eventBus.on(GameEvents.ENEMY_ARTIFICIER_DOT_ZONE_STARTED, (data: any) => {
      ...
      if (typeof zoneId === 'number') {
        this.activeArtificierZoneIds.add(zoneId);
      }
      if (this.activeArtificierZoneIds.size === 1) {
        ...
        try {
          ...
          console.warn('Failed to play artificier DOT zone hum sound:', err);
        }
      }
    });
    ...
    this.eventBus.on(GameEvents.ENEMY_ARTIFICIER_DOT_ZONE_ENDED, (data: any) => {
      ...
      const zoneId = data?.zoneId as number | undefined;
      if (typeof zoneId === 'number') {
        this.activeArtificierZoneIds.delete(zoneId);
      } else {
    ...
    this.eventBus.on(GameEvents.ENEMY_ARTIFICIER_ZONE_DAMAGE_TICK, (data: any) => {
      ...
      const position = data?.position || new Vector3(0, 0, 0);
      try {
        this.audioManager.playSoundAt('sfx_artificier_zone_dmg', position, 0.65);
      } catch (err) {
        console.warn('Failed to play artificier zone damage sound:', err);
      }
    });

    this.eventBus.on(GameEvents.ENEMY_HEALER_HEAL_CAST, (data: any) => {
      if (!this.audioManager) return;
      const position = data?.targetPosition || data?.casterPosition || new Vector3(0, 0, 0);
      try {
        this.audioManager.playSoundAt('sfx_healer_heal_beam1', position, 0.8);
      } catch (err) {
        console.warn('Failed to play healer heal beam sound:', err);
      }
    });
    ...
    this.eventBus.on(GameEvents.ENEMY_ROCKET_SENTRY_FIRED, (data: any) => {
      ...
      const position = data?.position || new Vector3(0, 0, 0);
      try {
        this.audioManager.playSoundAt('sfx_rocket_sentry_launched1', position, 0.85);
      } catch (err) {
        console.warn('Failed to play rocket sentry launch sound:', err);
      }
    });
    ...
    this.eventBus.on(GameEvents.ENEMY_ROCKET_SENTRY_IMPACT, (data: any) => {
      ...
      try {
        ...
        console.warn('Failed to play rocket sentry impact sound:', err);
      }
    });
    ...
    this.eventBus.on(GameEvents.ENEMY_SENTRY_SHOOTER_FIRED, (data: any) => {
      ...
      const position = data?.position || new Vector3(0, 0, 0);
      try {
        this.audioManager.playSoundAt('sfx_sentry_shots_fired1', position, 0.8);
      } catch (err) {
        console.warn('Failed to play sentry shooter fired sound:', err);
      }
    });
    ...
    this.eventBus.on(GameEvents.ENEMY_SENTRY_SHOOTER_PROJECTILE_FLIGHT_STARTED, (data: any) => {
      ...
      if (this.activeSentryShooterProjectilesInFlight === 1) {
        ...
        try {
          ...
          console.warn('Failed to play sentry projectile flying sound:', err);
        }
      }
    });
    ...
    this.eventBus.on(GameEvents.ENEMY_SENTRY_SHOOTER_ONHIT_PLAYER, (data: any) => {
      if (!this.audioManager) return;
      const position = data?.position || this.playerController?.getPosition?.() || new Vector3(0, 0, 0);
      try {
        this.audioManager.playSoundAt('sfx_sentry_onhit_sntry', position, 0.8);
      } catch (err) {
        console.warn('Failed to play sentry on-hit sound:', err);
      }
    });

    this.eventBus.on(GameEvents.ENEMY_NECROMANCER_SUMMON, (data: any) => {
      if (!this.audioManager) return;
      const position = data?.summonPosition || data?.casterPosition || new Vector3(0, 0, 0);
      try {
        this.audioManager.playSoundAt('sfx_necromancer_spawn', position, 0.85);
      } catch (err) {
        console.warn('Failed to play necromancer summon sound:', err);
      }
    });
    ...
    this.eventBus.on(GameEvents.ENEMY_DIED, (data: any) => {
      if (!this.audioManager) {
        console.warn('[Audio] AudioManager not available for death sound');
        return;
      }
      
      const enemyType = data?.enemyType as string;
      console.log('[Audio] ENEMY_DIED event received:', { enemyType, audioManagerReady: !!this.audioManager });
      
      if (!enemyType) {
        console.warn('[Audio] No enemyType in ENEMY_DIED event');
        return;
      }
      ...
      const soundMap: Record<string, string> = {
        ...
        'jumper': 'sfx_jumper_dmg_taken2',
        'jumper_boss': 'sfx_jumper_dmg_taken2',
      ...
      const sound = soundMap[enemyType] || (enemyType?.includes('pong') ? 'sfx_pong_ondeath' : undefined);
      console.log('[Audio] Death sound lookup:', { enemyType, sound, hasAudioManager: !!this.audioManager });
      ...
      if (sound) {
        try {
          ...
          if (
            enemyType === 'sentinel' ||
            enemyType === 'prefire_sentinel' ||
            enemyType === 'healer' ||
            enemyType === 'artificer' ||
            enemyType === 'artificier' ||
            enemyType === 'rocket_sentry' ||
            enemyType === 'swarm_coordinator' ||
            enemyType === 'necromancer' ||
            enemyType === 'laser_patterns' ||
            enemyType === 'mage_missile' ||
            enemyType === 'bullet_hell'
          ) {
            this.audioManager.fadeOutAndStopSound('sfx_artificier_splash_zone_hum1', 120);
          }
          if (enemyType === 'bull' || enemyType === 'bull_boss') {
            this.audioManager.stopAllSounds([sound]);
          }
          console.log(`[Audio] Attempting to play death sound: ${sound}`);
          this.audioManager.playSoundAt(sound, position, 0.8);
          console.log(`[Audio] Successfully triggered death sound ${sound}`);
        } catch (err) {
          console.warn(`Failed to play monster death sound ${sound}:`, err);
        }
      } else {
        console.warn(`[Audio] Cannot play death sound - no mapping for enemy type: ${enemyType}`);
      }
    });
  }
  ...
  private playMonsterAttackSound(attackerType: string): void {
    if (!this.audioManager) {
      console.warn('[Audio] AudioManager not available for attack sound');
      return;
    }
    ...
    console.log('[Audio] playMonsterAttackSound called:', { attackerType, soundsAvailable: sounds.length });
    
    if (sounds.length > 0) {
      const randomSound = sounds[Math.floor(Math.random() * sounds.length)];
      try {
        console.log(`[Audio] Playing ${randomSound} for ${attackerType} attacking player`);
        this.audioManager.playSoundAt(randomSound, this.playerController?.getPosition?.() || new Vector3(0, 0, 0), 0.8);
      } catch (err) {
        console.warn(`Failed to play monster attack sound ${randomSound}:`, err);
      }
    } else {
      console.warn(`[Audio] No attack sounds mapped for attacker type: ${attackerType}`);
    }
  }
  ...
  private handleAttackPerformedEvent(data: AttackPerformedPayload): void {
    ...
    if (data?.type === 'melee' && data?.attacker) {
      ...
      if (finalDamage > 0) {
        ...
        // Play monster attack sound when they successfully damage the player
        if (attackerType && this.audioManager) {
          ...
          if (attackerType.includes('pong')) {
            this.audioManager.playSoundAt(
              'sfx_pong_damagedealt',
              this.playerController?.getPosition?.() || new Vector3(0, 0, 0),
              0.75
            );
          }
        }
  ...
  private loadRoomByIndex(index: number, options?: { preferPreparedEnemies?: boolean }): void {
    ...
    this.activeArtificierZoneIds.clear();
    this.activeArtificierProjectilesInFlight = 0;
    this.activeSentryShooterProjectilesInFlight = 0;
    this.audioManager?.stopSound('sfx_artificier_splash_zone_hum1');
    this.audioManager?.stopSound('sfx_sentry_projectile_flying1');
